...
- **`017-unreadable-meta-data` stays `malformed`, and that is correct.** The xref rebuild in §4.1 repairs its broken table, but objects 79–174 are absent from the file, including object 172 — the page tree root. pdftotext, pdfinfo and pypdf all fail on it too, pypdf after performing the same rebuild. Do not treat it as a bug to fix.
...
- `TestCorpusDeterminism` — every file converted twice through separately constructed Converters, byte-identical
